...
export default function ChatInput(
  ...
  const handleLanguageChange = async (newLanguage: string) => {
    ...
    try {
      await axios.post('http://172.16.123.27:8888/set-language', { language: newLanguage });
 ...
 const convertBlobToAudioBuffer = async (blob: { arrayBuffer: () => any; }) => {
  ...
  const handleUploadAudio = async (audioBlob: Blob) => {
    try {
      ...
      const response = await axios.post('http://172.16.123.27:8888/transcribe', formData, {
        headers: {
  ...
  const handleUploadImageFile = async (file: File) => {
    ...
    const apiUrl = 'http://172.17.11.52:8888/upload';
