...
async function getLanguage() {
  ...
  async function searchQuery(query: string) {
    try {
      const response = await fetch('http://127.0.0.1:5000/query', { // Replace with your Flask API URL
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async function getImageFromServer() {
    try {
      const response = await fetch('http://172.17.11.52:8888/get_image_llm'); // Replace with your Flask API URL
      const data = await response.json();
      if (data.image === 0) {
...
// console.log("propmt start");
  ...
  let RagPrompt = " Give the Disease name based on the following relevant information you got \n";
  ...
  if (highScoreResults.length > 0) {
    ...
    highScoreResults.slice(0, 3).forEach((result: SearchResult) => {
      ...
      RagPrompt += "Here is the doctor Name add this also for recomendation and last provide this doctor name as a doctor recomendation";
      RagPrompt += `${result.Doctor_name}. \n`;
    });
